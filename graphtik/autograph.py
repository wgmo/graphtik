...
"""
Harvest :term:`dependencies <dependency>` annotated callables to form :term:`pipeline`\\s.
"""
...
def autographed(
    ...
    *,
    ...
    returns_dict=UNSET,
    aliases=UNSET,
    inp_sideffects=UNSET,
    out_sideffects=UNSET,
    ...
    kws.update(
        {
            ...
            if v is not UNSET and k not in "kws fn name domain".split()
        }
    )
    ...
    **Example:**
    ...
    [FnOp(name='autographed',
        needs=['fn'(>'other_fn'),
            ...
            'domain'(?)],
        fn='autographed'),
    FnOp(name='get_autograph_decors',
        needs=['fn',
            'default'(?),
            'domain'(?)],
    ...
    def _apply_renames(
        self,
        rename_maps: Iterable[Union[Mapping, "UNSET"]],
        word_lists: Iterable,
    ):
        """
        Rename words in all `word_lists` matching keys in `rename_maps`.
        """
        rename_maps = [d for d in rename_maps if d and d is not UNSET]
